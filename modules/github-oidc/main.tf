...
resource "aws_iam_role" "terraform" {
  name = "skyrunna-${var.env}-terraform"
  ...
  assume_role_policy = jsonencode({
    ...
    Statement = [{
      ...
      Condition = {
        ...
        StringLike   = { "token.actions.githubusercontent.com:sub" = "repo:vincentainatech/Skyrunna-IaaC:*" }
      }
    }]
  })
}
...
resource "aws_iam_role_policy" "terraform_iam" {
  name = "skyrunna-${var.env}-terraform-iam"
